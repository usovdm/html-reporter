...
exports.getInitialState = (state, payload) => {
    const {
...
function reducer(state, action) {
    switch (action.type) {
        case actionNames.VIEW_INITIAL: {
            return exports.getInitialState(state, {...compiledData, ...action.payload});
        }
...
function createTestResultsFromDb(state, action) {
    const {db, fetchDbDetails, testsTreeBuilder} = action.payload;
