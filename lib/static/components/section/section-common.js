...
import {hasFails, hasFailedRetries, shouldSuiteBeShown, shouldBrowserBeShown} from '../../modules/utils';
...
class SectionCommon extends Component {
    ...
    _getStates(props = this.props) {
        ...
        return {
            ...
            retried: hasFailedRetries(suite),
            expand
        };
    }
    ...
    render() {
        ...
        if (!opened) {
            return (
                <div className={sectionStatusResolver({status, opened, sectionRoot})}>
        ...
        const childrenTmpl = visibleChildren.map((child) => {
            ...
            return <SectionCommonWrapper
                ...
                strictMatchFilter={strictMatchFilter}
                filteredBrowsers={filteredBrowsers}
                errorGroupTests={errorGroupTests}
                sectionRoot={false}
            />;
        });
